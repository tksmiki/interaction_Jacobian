---
title: "R Notebook for MDR S-map for DE and ODE models version 3"
...
Conducting smap with the optimal theta
```{r}
...
```{r}
gg_HP_result01 <- data.frame(time = HP_result$t[-final_ID], smap = HP_m_smap_res2_s$smap_coefficients[[1]]$c_1[-final_ID] - delta_11, type = rep("H1toH1", 199))
...
gg_HP_result <- rbind.data.frame(gg_HP_result01, gg_HP_result02)
...
　geom_line(aes(x = time, y = smap, group = type, color = type)) +
  ...
  xlim(1800, 1999) +  ylim(-0.5, 0.3) + theme_bw() + theme(legend.position = "bottom") + 
  labs(title = "Figure 1: Time evolution of S-map coefficients", x = "time", y = "Adjusted S-map coefficients") 
plot(fig1_ggplot)
...
for(i in 1:as.integer((end_time - transient_period) / deltat)){
  nv <- rk4(in_vec = nv, time = t, h_interval = deltat, dim = dim_model1, diff_vec = diff_5sp)
  t <- t + deltat # update time
  ###recored the result every "tau only
...
```
##### [10.1.3] Comparison for the diagonal element (Fig. 3cd: R -> C1): ggplotplot
